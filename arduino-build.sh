...
PROJECT=workshop-iotc-arduino-portenta
...
patch-mbed-2-5-2()
{
    sed -i -e "s/recipe.hooks.linking.prelink.1.pattern=\"{compiler.path}{compiler.c.elf.cmd}\" -E -P -x c {build.extra_flags}/recipe.hooks.linking.prelink.1.pattern=\"{compiler.path}{compiler.c.elf.cmd}\" -E -P -x c/g" ~/Library/Arduino15/packages/arduino/hardware/mbed_portenta/2.5.2/platform.txt
}
